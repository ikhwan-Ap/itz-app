...
export default function FullLatihan() {
  ...
  useEffect(() => {
    api.get("/calculator/meta").then((r) => {
      setMeta(r.data);
      // Check for resume session from history
      const resumeRaw = sessionStorage.getItem("resume_session");
      if (resumeRaw) {
        try {
          const data = JSON.parse(resumeRaw);
          if (data.mode === "full" && data.stats) {
            const init = {};
            (r.data.all_attrs || []).forEach((a) => (init[a] = data.stats[a] ?? 1));
            setStats(init);
            if (Array.isArray(data.roles) && data.roles.length) setActiveRoles(data.roles.filter((x) => x !== "GK"));
  ...
  const run = async () => {
    ...
    setLoading(true);
    try {
      const data = await runCalculator({ activeRoles, stats, bonus, greyLimit, targets, playerAge, whiteMultiplier });
      setResult(data);
      setStep(3);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 mb-1">
            <Target size={18} className="text-[#38BDF8]" weight="fill" />
            <div className="badge badge-gold">FULL SEARCH</div>
          </div>
          <h1 className="section-title text-3xl">Full Latihan</h1>
          <p className="text-[#A0AAB5] text-sm mt-1">Algoritma sniper multi-prioritas menemukan kombinasi drill paling hemat.</p>
        </div>
      </div>
