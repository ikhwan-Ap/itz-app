        ...
        {items.map((p) => (
          <div key={p.id} className="card-solid p-5">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-xs uppercase tracking-widest text-[#9BA4B5]">{p.duration_type}</div>
            ...
            <div className="font-display font-black text-3xl text-[#00D05E] mt-2">{formatRupiah(p.price)}</div>
            <div className="text-xs text-[#9BA4B5] mt-1">{p.description}</div>
            <div className="text-xs text-[#9BA4B5] mt-2">Durasi: {p.duration_value} {p.duration_type === "yearly" ? "tahun" : "bulan"}{p.max_clicks ? ` · ${p.max_clicks} clicks` : ""}</div>
    ...
    <div className="popup-overlay" onClick={onClose}>
      <div className="card-glass p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="text-[#9BA4B5]"><X size={20} /></button>
        </div>
