...
export default function AdminDashboard() {
  ...
  const kpis = [
    { label: "Total Users", value: stats.total_users, icon: Users, color: "#00A8FF" },
    { label: "Active", value: stats.active_users, icon: CheckCircle, color: "#00A8FF" },
    { label: "Pending", value: stats.pending_users, icon: Clock, color: "#F5C300" },
    { label: "Expiring 7D", value: stats.expiring_soon, icon: Warning, color: "#E50914" },
  ];
  const fin = [
    { label: "Gross Revenue", value: formatRupiah(stats.gross), icon: ChartBar, color: "#00A8FF" },
    ...
    { label: "Total Discount", value: formatRupiah(stats.discount_total), icon: CreditCard, color: "#E50914" },
  ];
  ...
  return (
    <div className="space-y-6" data-testid="admin-dashboard">
      <div>
        ...
        <p className="text-[#A0AAB5] text-sm mt-1">Global overview · Users · Revenue · Pending approvals</p>
      </div>
        ...
        {kpis.map((k, i) => (
          <div key={i} className="card-solid p-4 sm:p-5" data-testid={`kpi-${k.label.toLowerCase().replace(/\s+/g, "-")}`}>
            <div className="flex items-center justify-between">
              <div className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-[#A0AAB5]">{k.label}</div>
        ...
        <div className="card-solid p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={stats.chart}>
              ...
              <Legend />
              <Bar dataKey="gross" fill="#00A8FF" name="Gross" />
        ...
        <div className="card-solid p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={stats.chart}>
              ...
              <Tooltip contentStyle={{ background: "#161B22", border: "1px solid #1F2630" }} />
              <Legend />
              <Line type="monotone" dataKey="net" stroke="#00A8FF" strokeWidth={3} />
              ...
              <tbody>
                {stats.expiring_list.map((u) => (
                  <tr key={u.id} className="hover-row border-t border-white/5">
                    ...
                    <td className="text-[#A0AAB5]">{u.email}</td>
            ...
            {stats.expiring_list.map((u) => (
              <div key={u.id} className="p-3 rounded-lg bg-white/[0.03] border border-white/5 flex items-center justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-[#A0AAB5] truncate">{u.email}</div>
                </div>
            ...
            <tbody>
              {stats.recent_tx.map((t) => (
                <tr key={t.id} className="hover-row border-t border-white/5">
                  <td className="py-2">{t.user_name}<div className="text-xs text-[#A0AAB5]">{t.user_email}</div></td>
                  ...
                  <td className="text-[#A0AAB5]">{new Date(t.created_at).toLocaleDateString("id-ID")}</td>
                </tr>
              ))}
            </tbody>
          </table>
