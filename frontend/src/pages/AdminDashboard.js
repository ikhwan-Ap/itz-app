...
export default function AdminDashboard() {
  ...
  const kpis = [
    { label: "Total Users", value: stats.total_users, icon: Users, color: "#00D05E" },
    { label: "Active", value: stats.active_users, icon: CheckCircle, color: "#00D05E" },
    { label: "Pending", value: stats.pending_users, icon: Clock, color: "#F5C300" },
    { label: "Expiring 7D", value: stats.expiring_soon, icon: Warning, color: "#FF3366" },
  ];
  const fin = [
    { label: "Gross Revenue", value: formatRupiah(stats.gross), icon: ChartBar, color: "#00D05E" },
    ...
    { label: "Total Discount", value: formatRupiah(stats.discount_total), icon: CreditCard, color: "#FF3366" },
  ];
  ...
  return (
    <div className="space-y-6" data-testid="admin-dashboard">
      <div>
        ...
        <p className="text-[#9BA4B5] text-sm mt-1">Global overview · Users · Revenue · Pending approvals</p>
      </div>
        ...
        <div className="card-solid p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={stats.chart}>
              ...
              <Legend />
              <Bar dataKey="gross" fill="#00D05E" name="Gross" />
        ...
        <div className="card-solid p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={stats.chart}>
              ...
              <Tooltip contentStyle={{ background: "#131E35", border: "1px solid #1C2B4B" }} />
              <Legend />
              <Line type="monotone" dataKey="net" stroke="#00D05E" strokeWidth={3} />
            ...
            <tbody>
              {stats.recent_tx.map((t) => (
                <tr key={t.id} className="hover-row border-t border-white/5">
                  <td className="py-2">{t.user_name}<div className="text-xs text-[#9BA4B5]">{t.user_email}</div></td>
                  ...
                  <td className="text-[#9BA4B5]">{new Date(t.created_at).toLocaleDateString("id-ID")}</td>
                </tr>
              ))}
            </tbody>
          </table>
